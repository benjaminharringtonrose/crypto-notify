...
Technical Indicators → Pattern Detection → Feature Computation
  ...
  ATR, VWAP, OBV    Triple Top/Bottom  (BTC: 36 features)
```
...
#### BTC Features (Updated based on current implementation)

The current FeatureCalculator `compute` method generates features based on the `isBTC` parameter. For Bitcoin (when `isBTC` is true or not specified), it generates comprehensive technical analysis features.

```typescript
// The actual feature computation includes:
// 1. Technical indicators (RSI, MACD, SMA, etc.)
// 2. Pattern detection (Double Top, Head & Shoulders, etc.)
// 3. Market regime features (volatility, trend, momentum regimes)
// 4. Advanced microstructure features (Ichimoku, Williams %R, etc.)
// 5. Normalized ratios and derived features
```

**Note**: The exact feature list is dynamically generated by the `calculateIndicators` method and includes over 60 comprehensive features covering all major technical analysis categories.
