...
// Improved Random Forest implementation with better regularization
class RandomForestModel extends BaseModel {
  ...
  constructor(
    nEstimators: number = 20, // Further reduced for better generalization
    maxDepth: number = 3, // Further reduced to prevent overfitting
    minSamplesSplit: number = 30, // Increased for more robust splits
    maxFeatures: number = 0.5, // Use 50% of features per tree
    minSamplesLeaf: number = 15 // Increased minimum samples per leaf
  ) {
    super("random_forest");
...
class LogisticRegressionModel extends BaseModel {
  ...
  private regularization: number = 0.01; // L2 regularization
  ...
  async train(X: number[][], y: number[]): Promise<void> {
    ...
    const { scaledX, mean, std } = this.standardizeFeatures(X);

    for (let iteration = 0; iteration < this.maxIterations; iteration++) {
      let totalLoss = 0;
      let totalGradientW = new Array(nFeatures).fill(0);
      let totalGradientB = 0;
  ...
  predict(X: number[][]): number[] {
    ...
    return X.map((sample) => {
      const scaledSample = this.scaleFeatures(sample);
      const probability = this.sigmoid(
        this.dotProduct(scaledSample, this.weights) + this.bias
      );
  ...
  private standardizeFeatures(X: number[][]): {
    scaledX: number[][];
    mean: number[];
    std: number[];
  } {
...
export class BitcoinPredictor {
  ...
  async trainModels(
    ...
    await this.optimizeEnsembleWeights(X, y);
      ...
      try {
        ...
        for (let fold = 0; fold < cvFolds; fold++) {
          ...
          if (trainX.length > 0 && testX.length > 0) {
            ...
            const predictions = tempModel.predict(testX);
            const accuracy = this.calculateAccuracy(testY, predictions);
            cvScores[name].push(accuracy);
          }
        }
      } catch (e) {
        console.error(`Error in cross-validation for ${name}: ${e}`);
        cvScores[name] = [0.5]; // Default to random performance
      }
    }
  ...
  private createModelCopy(name: string, originalModel: BaseModel): BaseModel {
    // Create a copy of the model for cross-validation
    switch (name) {
      case "random_forest":
        return new RandomForestModel(20, 3, 30, 0.5, 15);
